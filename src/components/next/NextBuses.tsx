...
import { Badge, EmptyState, Icon } from "@/components/ui/primitives";
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0">
          <p className="truncate text-sm font-bold">{data.route.longName}</p>
      ...
      <ol className="divide-y divide-line rounded-card border border-line bg-paper-2">
        {data.next.map((n, i) => {
          ...
          const tone = n.source === "live" ? "live" : n.source === "estimated" ? "info" : "neutral";
          return (
            <li key={`${n.tripId ?? i}-${n.time}`} className="flex items-center gap-3 px-3 py-2.5">
              <span className="grid h-11 w-14 shrink-0 place-items-center rounded-lg text-white" style={{ background: ETA_COLORS[b] || "var(--ink-3)" }}>
                <span className="text-lg font-extrabold leading-none tabular-nums">{n.minutes <= 0 ? "•" : n.minutes}</span>
                <span className="text-[10px] font-semibold">{n.minutes <= 0 ? t.next.arrivingNow : t.common.min}</span>
              </span>
              <div className="min-w-0 flex-1">
                <p className="flex flex-wrap items-center gap-1.5 text-sm">
                  <Badge tone={tone}>
                    {n.source === "live" ? <span className="live-dot" style={{ width: 5, height: 5 }} /> : null}
                    {n.source === "live" ? t.next.live : n.source === "estimated" ? t.next.estimated : t.next.scheduled}
                  </Badge>
