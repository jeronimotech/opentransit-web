...
- **Planner form**: one time control (`Ahora ▾` → Salir a las / Llegar antes de + picker), one mode row that fits (Bus · Cable · Bici · A pie), advanced toggles under *Más opciones*, CTA pinned at the bottom. Every search result can become the origin or the destination, both fields offer *Mi ubicación* and *Elegir en el mapa*, and the swap between them works with one field empty.
- **Elegir en el mapa**: a full-screen map with a fixed centre crosshair; the centre is reverse geocoded while you pan (debounced) and shown in the confirm bar, falling back to coordinates — a point with no name is still a valid endpoint.
